...
parser.add_argument(
    "--h",
    dest="hide",
    help="Show hidden files and directories (bool)",
    action="store_false",
)
parser.add_argument(
    "--l",
    ...
    type=int,
    default=1,
    help="Level of depth to print (int)",
)
...
class TreeGrower:
    ...
    def generate_tree(self, d, max_indent=float("inf"), indent=0) -> list:
        ...
        for i, item in enumerate(d_contents):
            ...
            branch += [f"{extension}{stem}{name}{slash}"] + self.generate_tree(
                item, max_indent, indent + 1
            )  # noqa
        return branch
...
def main():
    ...
    root = Path.cwd()
    root_str = str(root)
    root = TreeGrower(root_str, args.hide, args.level)
